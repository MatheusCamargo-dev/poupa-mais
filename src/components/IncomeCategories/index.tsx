...
interface Props {
  ...
  append: ({ incomeCategory }: {incomeCategory: string}) => void;
...
export default function IncomeCategories(props: Props) {

  const { newCategories, append, remove, register, error } = props;
  const [showCategories, setShowCategories] = useState(false);

  const addNewCategory = () => {
    append({ incomeCategory: ''})
  }
  return (
      ...
      <div className='flex flex-col'>
        ...
        </div>
        {
          ...
          <div className="flex flex-col space-y-2 mt-4">
            {
              newCategories.map((field: {id: React.Key, incomeCategory: string}, index: number) => {
                return (
                  <div key={field.id} className='space-y-2'>
                    <div className='flex justify-between p-1 bg-white rounded-md border-2 border-gray-300'>
                            ...
                            <input type='text' maxLength={24} {...register(`incomeCategories.${index}.incomeCategory`)} defaultValue={field.incomeCategory} className='w-full border-b-2 p-1 focus:outline-none border-gray-300 text-blue-400 text-xl' />
                          </h1>
                          <TbTrashFilled
                            size={30}
                      ...
                      {error && error[index] && <ErrorMessage errorMessage={error[index]?.incomeCategory}></ErrorMessage>}
                  </div>
                );
              })
            }
